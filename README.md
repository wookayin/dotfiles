...
- Add `summon`, `granted(assume)` installation script, alias for aws credentials
...
On Linux, you can [install some common softwares locally][linux-locals.sh] (into `$HOME/.local/bin`) _without sudo_:
...
_Please read carefully warning messages during installation !!_
...
- If something goes wrong, please run **[`$ dotfiles update`][dotfiles-update]** (or [install.py]) to make everything up-to-date.
  ...
  - If you have your own `~/.zshrc`, `~/.vimrc`, `~/.vim`, etc., that are NOT symbolic links,
    ...
    In such cases you should delete these files _manually_.
...
- If [**neovim**][neovim] emits any startup errors (e.g. `no module named neovim`):
  ...
  - Make sure that the [`pynvim`](https://pypi.python.org/pypi/pynvim/) pypi package is installed on _local_ python 3,
