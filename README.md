...
- If neovim cannot run due to `version 'GLIBC_2.29' not found` errors (on Ubuntu 18.04 or earlier),
  you should upgrade your Ubuntu distribution to 20.04+ in order to run recent versions of nvim.
  If you use an [appimage](https://github.com/neovim/neovim/releases/tag/stable) binary of neovim,
...
- If [**neovim**][neovim] emits any startup errors (e.g. `no module named neovim`):
    - Use **latest neovim** (e.g., neovim 0.12.1).
    ...
    - We require python3 version >= 3.6. See <https://endoflife.date/python>
...
- Does vim color look weird (e.g. only black-and-white)?
  - Check whether your terminal emulator supports [24-bit color](https://github.com/wookayin/dotfiles/pull/9).
